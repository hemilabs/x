...
func randomStorageSet(accounts []string, hashes [][]string, nilStorage [][]string) map[common.Hash]map[common.Hash][]byte {
	...
	for index, account := range accounts {
		...
		if index < len(hashes) {
			hashes := hashes[index]
			for _, hash := range hashes {
				storages[common.HexToHash(account)][common.HexToHash(hash)] = testutil.Random(32)
			}
		}
...
func TestAccountIteratorBasics(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		hash := testutil.Hash()
		data := testutil.Random(32)
		accounts[hash] = data

		if rand.Intn(2) == 0 {
			accStorage := make(map[common.Hash][]byte)
			accStorage[testutil.Hash()] = testutil.Random(32)
...
func TestStorageIteratorBasics(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		hash := testutil.Hash()
		accounts[hash] = testutil.Random(32)
		...
		for i := 0; i < 100; i++ {
			if rand.Intn(2) == 0 {
				...
				accStorage[testutil.Hash()] = nil // delete slot
				nilstorage += 1
			}
		}
		storage[hash] = accStorage
