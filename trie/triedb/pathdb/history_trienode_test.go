...
import (
	...
	"github.com/hemilabs/x/trie/testutil"
)
...
func randomTrienodes(n int) (map[common.Hash]map[string][]byte, common.Hash) {
	...
	for i := 0; i < n; i++ {
		...
		for j := 0; j < 10; j++ {
			path := testutil.Random(rand.Intn(10))
			for z := 0; z < len(path); z++ {
				nodes[owner][string(path[:z])] = testutil.Random(rand.Intn(128))
			}
		}
		// zero-size trie node, representing it was non-existent before
		for j := 0; j < 10; j++ {
			path := testutil.Random(32)
			nodes[owner][string(path)] = nil
		}
		// root node with zero-size path
		rnode := testutil.Random(256)
		nodes[owner][""] = rnode
...
func TestSingleTrieHistory(t *testing.T) {
	...
	nodes[owner][""] = testutil.Random(32)      // empty key
	nodes[owner]["a"] = testutil.Random(1)      // small value
	nodes[owner]["bb"] = testutil.Random(100)   // medium value
	nodes[owner]["ccc"] = testutil.Random(1000) // large value
	nodes[owner]["dddd"] = testutil.Random(0)   // empty value
...
func TestMultipleTries(t *testing.T) {
	...
	owner1 := testutil.Hash()
	nodes[owner1] = make(map[string][]byte)
	for i := 0; i < 100; i++ {
		key := string(testutil.Random(rand.Intn(10)))
		nodes[owner1][key] = testutil.Random(rand.Intn(50))
	}

	// Second trie with few large nodes
	owner2 := testutil.Hash()
	nodes[owner2] = make(map[string][]byte)
	for i := 0; i < 5; i++ {
		key := string(testutil.Random(rand.Intn(20)))
		nodes[owner2][key] = testutil.Random(1000 + rand.Intn(1000))
	}

	// Third trie with nil values (zero-size nodes)
	owner3 := testutil.Hash()
	nodes[owner3] = make(map[string][]byte)
	for i := 0; i < 10; i++ {
		key := string(testutil.Random(rand.Intn(15)))
...
func TestLargeNodeValues(t *testing.T) {
	...
	for _, size := range sizes {
		key := string(testutil.Random(10))
		nodes[owner][key] = testutil.Random(size)
...
func TestTrienodeHistoryReaderNonExistentPath(t *testing.T) {
	...
	_, err = tr.read(testutil.Hash(), "nonexistent")
	if err == nil {
