...
import (
	...
	"github.com/hemilabs/x/trie/trie/trienode"
...
func testCommit(b *testing.B, n int, parallel bool) {
	...
	for i := 0; i < b.N; i++ {
		...
		for j := 0; j < n; j++ {
			key := testutil.Random(32)
			val := testutil.Random(32)
...
func TestTrieCopy(t *testing.T) {
	...
	var entries []kv
	for i := 0; i < 256; i++ {
		entries = append(entries, kv{k: testutil.Random(32), v: testutil.Random(32)})
	}
...
func testTrieCopyOldTrie(t *testing.T, entries []kv) {
	...
	for _, val := range entries {
		if rand.Intn(2) == 0 {
			...
			trCpy.Update(val.k, testutil.Random(32))
		}
	}
	for i := 0; i < 10; i++ {
		trCpy.Update(testutil.Random(32), testutil.Random(32))
	}
	trCpy.Hash()
	trCpy.Commit(false)
...
func testTrieCopyNewTrie(t *testing.T, entries []kv) {
	...
	for _, val := range entries {
		if rand.Intn(2) == 0 {
			...
			tr.Update(val.k, testutil.Random(32))
		}
	}
	for i := 0; i < 10; i++ {
		tr.Update(testutil.Random(32), testutil.Random(32))
	}
...
// goos: darwin
// goarch: arm64
// pkg: github.com/hemilabs/x/trie/trie/
// cpu: Apple M1 Pro
