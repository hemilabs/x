...
func makeTestSet(owner common.Hash, n int, paths [][]byte) *NodeSet {
	...
	for i := 0; i < n*3/4; i++ {
		path := testutil.Random(10)
		blob := testutil.Random(100)
		set.AddNode(path, NewNodeWithPrev(crypto.Keccak256Hash(blob), blob, testutil.Random(100)))
	}
	for i := 0; i < n/4; i++ {
		path := testutil.Random(10)
		set.AddNode(path, NewDeletedWithPrev(testutil.Random(100)))
	}
	for i := 0; i < len(paths); i++ {
		if i%3 == 0 {
			set.AddNode(paths[i], NewDeletedWithPrev(testutil.Random(100)))
		} else {
			blob := testutil.Random(100)
			set.AddNode(paths[i], NewNodeWithPrev(crypto.Keccak256Hash(blob), blob, testutil.Random(100)))
		}
	}
	return set
}
...
func TestNodeSetMerge(t *testing.T) {
	var shared [][]byte
	for i := 0; i < 2; i++ {
		shared = append(shared, testutil.Random(10))
	}
	owner := testutil.Hash()
