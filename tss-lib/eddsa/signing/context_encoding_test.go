...
// from round_2.go line 37 and verifies it uses length-prefixed encoding via
// AppendBigIntToBytesSlice, not bare append.
func TestContextIEncodingMatchesRound2(t *testing.T) {
	ssid := []byte("test-ssid-for-eddsa-signing-round2")

	for _, partyIndex := range []int{0, 1, 2, 255} {
		i := partyIndex
		// This is the exact pattern from round_2.go:37
...
// TestContextIGoldenVectorsEdDSASigning freezes the exact byte output of ContextI
// for known inputs, so any regression in AppendBigIntToBytesSlice is caught.
func TestContextIGoldenVectorsEdDSASigning(t *testing.T) {
	ssid := []byte("test-ssid")
	...
	for _, tc := range tests {
		// Exact pattern from round_2.go:37
